...
calculate_wtd_trajectories <- function(data_in=NULL){
...
head(data_in)
denom <- ddply(data_in, .(age), function(w) sum(w$stipw))
names(denom)[2] <- "denom"
remaining <- ddply(data_in, .(age), function(w) sum(w$instudy))
names(remaining)[2] <- "remaining"
cbind(remaining[,2], denom[,2])
long01.int <- join(data_in,denom, by="age")
long01 <- join(long01.int, remaining, by="age")
long01$stipw01 <- long01$stipw/long01$denom
long01$stipw01.n <- long01$remaining*long01$stipw01
long01$inches_wtd_hadamard <- long01$inches*long01$stipw01.n
long01$inches_wtd_hadamard[long01$inches_wtd_hadamard==0] <- NA  ## uncomment; 
